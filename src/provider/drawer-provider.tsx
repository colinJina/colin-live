...
export function DrawerProvider({ children }: { children: ReactNode }) {
  ...
  const openDrawer = useCallback(
    (content: ReactNode | string) => {
      ...
      if (typeof content === 'string') {
        ...
        } else if (hasHtml(text) || text.includes('<img') || text.toLowerCase().includes('src=')) {
          // HTML内容
          try {
            setContent(
              <div className="h-full w-full p-4">
  ...
  return (
    <DrawerContext.Provider value={{ openDrawer, closeDrawer,showMessage, drawerContent: content }}>
      {children}
      <Drawer
        title="功能说明"
        open={open}
