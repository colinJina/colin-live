...
export default function CategoryModule() {
    ...
    const onCategoryClick = (code?: string) => {
        if (!code) {
            return;
        }
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-start">
                    <button
                        ...
                        className={cn(
                            ...
                            hotActive
                                ? 'bg-[#fff2f5] text-[var(--color-bili-pink)]'
                    ...
                    >
                        <span
                            className={cn(
                                ...
                                hotActive
                                    ? 'bg-[var(--color-bili-pink)] text-white'
                                    : 'bg-[#ff7f8f] text-white',
                            )}
                        >
                            <FireIcon />
                        </span>
                        ...
                        {!isLoading && !isError && topCategories.length > 0 && (
                            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-10">
                                {topCategories.map((category) => {
                                    ...
                                    const children =
                                        category.children?.filter((item) => item.categoryName) ??
                                        [];
                                    const isChildActive = children.some(
                                        (item) =>
                                            item.categoryCode && item.categoryCode === activeCode,
                                    );
                                    const active = Boolean(
                                        (currentCode && currentCode === activeCode) ||
                                        isChildActive,
                                    );
                                    ...
                                    return (
                                        ...
                                        >
                                            <Button
                                                block
                                                type="text"
                                                onClick={() => onCategoryClick(currentCode)}
                                                disabled={!currentCode}
                                                className={cn(
                                                    '!h-10 !rounded-[10px] !border !text-[15px] !font-medium !transition-colors',
                                                    active
                                                        ? '!border-[var(--color-bili-pink)] !bg-[#fff2f5] !text-[var(--color-bili-pink)]'
